...
// Always-on ambient scene: floating polyhedra + torus rings + starfield,
// with mouse/scroll parallax. Orange + blue. Pauses on document.hidden.
// Device-tiered: A = full, B = trimmed, C = lite; skipped entirely for
// prefers-reduced-motion (the CSS glow orbs carry the ambience).
export default function BackgroundScene() {
  ...
  useEffect(() => {
    ...
    const starCount = tier === 'A' ? 1500 : tier === 'B' ? 600 : 350;
    const shapeCount = tier === 'A' ? 8 : tier === 'B' ? 5 : 4;
    const ringCount = tier === 'C' ? 0 : 3;
    ...
    const activeDefs = shapeDefs.slice(0, shapeCount);
    activeDefs.forEach((def, i) => {
      const mat = new THREE.MeshStandardMaterial({
        color: def.color, metalness: 0.6, roughness: 0.4, flatShading: true, transparent: true, opacity: 0.18,
      });
      const mesh = new THREE.Mesh(def.geo, mat);
      const edges = new THREE.EdgesGeometry(def.geo);
      const lineMat = new THREE.LineBasicMaterial({ color: def.edgeColor, transparent: true, opacity: 0.35 });
      const wire = new THREE.LineSegments(edges, lineMat);
      const group = new THREE.Group();
      group.add(mesh); group.add(wire);

      const angle = (i / activeDefs.length) * Math.PI * 2;
      ...
      group.userData = {
        ...
        basePos: group.position.clone(), edges, lineMat, mat,
      };
      scene.add(group);
      shapes.push(group);
    });
    ...
    for (let i = 0; i < ringCount; i++) {
      ...
      const torusMat = new THREE.MeshBasicMaterial({ color: i % 2 === 0 ? 0xff6b3d : 0x5b9eff, transparent: true, opacity: 0.15 });
      const torus = new THREE.Mesh(torusGeo, torusMat);
      torus.rotation.x = Math.random() * Math.PI;
      torus.rotation.y = Math.random() * Math.PI;
      torus.position.z = -3 - i * 2;
      torus.userData = { rotSpeed: { x: (Math.random() - 0.5) * 0.15, y: (Math.random() - 0.5) * 0.15 }, torusGeo, torusMat };
      scene.add(torus);
      rings.push(torus);
    }
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      if (tone < 0.6) { starColors[i * 3] = 1; starColors[i * 3 + 1] = 0.95; starColors[i * 3 + 2] = 0.9; }
      else if (tone < 0.85) { starColors[i * 3] = 1; starColors[i * 3 + 1] = 0.55; starColors[i * 3 + 2] = 0.3; }
      else { starColors[i * 3] = 0.4; starColors[i * 3 + 1] = 0.65; starColors[i * 3 + 2] = 1; }
    }
    starGeo.setAttribute('position', new THREE.BufferAttribute(starPositions, 3));
    starGeo.setAttribute('color', new THREE.BufferAttribute(starColors, 3));
    const starMat = new THREE.PointsMaterial({ size: 0.06, transparent: true, opacity: 0.7, vertexColors: true, blending: THREE.AdditiveBlending, depthWrite: false });
    const stars = new THREE.Points(starGeo, starMat);
    scene.add(stars);

    scene.add(new THREE.AmbientLight(0xffffff, 0.4));
    const keyLight = new THREE.PointLight(0xff6b3d, 4, 40); keyLight.position.set(8, 6, 5); scene.add(keyLight);
    const fillLight = new THREE.PointLight(0x5b9eff, 3, 40); fillLight.position.set(-8, -4, 5); scene.add(fillLight);
    const rimLight = new THREE.PointLight(0xffffff, 1.5, 30); rimLight.position.set(0, 8, -8); scene.add(rimLight);
    ...
    const animate = (now) => {
      ...
      rings.forEach((r) => { r.rotation.x += r.userData.rotSpeed.x * 0.01; r.rotation.y += r.userData.rotSpeed.y * 0.01; });
    ...
    return () => {
      ...
      shapes.forEach((g) => { g.userData.edges.dispose(); g.userData.lineMat.dispose(); g.userData.mat.dispose(); });
